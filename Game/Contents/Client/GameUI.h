...
private:
	...
	void DrawMain()
	{
		static bool showIconList = false;
		...
		ImGui::Text(u8"점수 : %d", client_->myPlayer_.rating_);
		...
		if (ImGui::BeginPopupModal(u8"아이콘 목록", &showIconList, ImGuiWindowFlags_NoResize))
		{
			...
			for (const Icon& icon : client_->myPlayer_.icons_)
			{
				ImGui::Text(u8"Icon ID : %d", icon.iconId_);

				ImGui::SameLine();

				// 실제 장착 여부는 Inventory ID로 판단
				if (icon.inventoryId_ == client_->myPlayer_.equippedInvenId_)
				{
					ImGui::Text(u8"[장착 중]");
				}
				else
				{
					...
					ImGui::PushID(icon.inventoryId_);

					if (ImGui::Button(u8"선택", ImVec2(80, 30)))
					{
						g_ClientRpcProxy.ReqChangeEquipment(icon.inventoryId_);
					}
		...
		for (auto& p : client_->playerMap_)
		{
			...
			ImGui::Text(u8"%-10s  %4dpt   %s", p.second.playerName_.c_str(), p.second.rating_, playerState);
		}
